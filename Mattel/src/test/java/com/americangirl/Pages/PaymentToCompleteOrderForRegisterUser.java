...
public class PaymentToCompleteOrderForRegisterUser {

	WebDriver driver;
	private PageHelper helper;
	
	public PaymentToCompleteOrderForRegisterUser(WebDriver ldriver) {
		this.driver = ldriver;
...
public void enterCreditCardPayment() throws Exception {
		...
		(new WebDriverWait(driver,10)).until(ExpectedConditions.visibilityOf(ReviewOrderButton));
	...
	public void completeOrder() {
		...
		(new WebDriverWait(driver,15)).until(ExpectedConditions.visibilityOfElementLocated(By.id("payment")));
		helper.waitForClickable(CompleteOrder);
		CompleteOrder.click();
	}
	
	public void orderConfirmationPage() {
		(new WebDriverWait(driver,12)).until(ExpectedConditions.visibilityOf(ConfirmationText));
